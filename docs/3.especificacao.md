...
|--------------------|------------------------------------|----------------------------------------|
| RF1 | Cadastro de Usuário |	O sistema deve permitir que novos usuários realizem seu cadastro informando nome, e-mail e senha. |
| RF2 | Autenticação de Usuário	| O sistema deve permitir que usuários autenticados acessem o sistema por meio de login com e-mail e senha. |
| RF3	| Registro de Receitas |	O sistema deve permitir o cadastro de receitas, informando valor, data e descrição.  |
| RF4	| Registro de Despesas |	O sistema deve permitir o cadastro de despesas, informando valor, data e descrição.  |
| RF5	| Consulta de Movimentações |	O sistema deve permitir a visualização das receitas e despesas registradas.  |
| RF6	| Filtro de Movimentações |	O sistema deve permitir filtrar movimentações por período ou tipo (receita/despesa).  |
| RF7	| Visualização de Fluxo de Caixa |	O sistema deve apresentar o saldo financeiro com base nas receitas e despesas registradas.  |
| RF8	| Geração de Relatórios |	O sistema deve permitir a geração de relatórios financeiros com base nas movimentações cadastradas.  |
...
|--------|--------------------------|
| RNF1 | O sistema deve possuir interface simples e intuitiva, facilitando o uso por usuários não técnicos. |
| RNF2 | O sistema deve ser acessível via navegador web, sem necessidade de instalação. |
| RNF3 | O sistema deve ser compatível com os principais navegadores (Chrome, Firefox e Edge). |
| RNF4 | O carregamento das páginas e funcionalidades deve ocorrer em até 3 segundos. |
| RNF5 | O acesso ao sistema deve ser protegido por autenticação com login e senha. |
| RNF6 | Os dados inseridos devem ser armazenados de forma segura e consistente, evitando perdas ou duplicações. |
| RNF7 | O sistema deve manter disponibilidade mínima, evitando falhas que impeçam o uso contínuo. |
| RNF8 | O código do sistema deve ser organizado e de fácil manutenção para fins acadêmicos. |
